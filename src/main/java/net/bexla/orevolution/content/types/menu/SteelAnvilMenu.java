...
public class SteelAnvilMenu extends ItemCombinerMenu {
    ...
    public SteelAnvilMenu(int containerId, Inventory playerInventory) {
        this(containerId, playerInventory, ContainerLevelAccess.NULL);
    }

    public SteelAnvilMenu(int containerId, Inventory playerInventory, ContainerLevelAccess access) {
        super(RegMenus.STEEL_ANVIL.get(), containerId, playerInventory, access);
    ...
    public void createResult() {
        ...
        if (!itemstack.isEmpty()) {
            if (!onAnvilChange(this, itemstack, this.inputSlots.getItem(1), resultSlots, itemName, j, this.player)) {
                return;
            }
        }
        if (!itemstack.isEmpty() && EnchantmentHelper.canStoreEnchantments(itemstack)) {
            ...
            int k2 = (int)Math.floor((j + (long)i) * 0.7D);

            if (i > 0 && k2 <= 0) {
                k2 = 1;
            }

            k2 = Mth.clamp(k2, 0, Integer.MAX_VALUE);
            this.cost.set(k2);
            if (i <= 0) {
